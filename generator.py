...
class Event:
    def __init__(self, state_, time_, packet_, number_, port_):
...
class Switch:
    def __init__(self, number_):
        ...
        self.port_dict = dict()         # список выходных портов на коммутаторе  


class Port:
    def __init__(self, id_, bandwidth_):
        self.id = id_                       # идентификатор порта в виде (start, end)
...
def create_virtual_send(topology):
    for key in topology.keys():
        ...
        for port in sw.port_dict.values():
            for queue_key in port.queues_info.keys():
                if port.queues_info[queue_key].priority > max_queue_priority:
                    max_queue_priority = port.queues_info[queue_key].priority
            for i in range(0, max_queue_priority):
                port.queues_send.append(list())
...
def parse_config(argv, slices, topology):
    ...
    with open(argv[0]) as json_file:
        ...
        for sls in slices_data:
            ...
            i = 1
            for fl in sls['flows']:
                flow = Flow(i, fl['lambda'], fl['path'], sls['sls_number'])
        ...
        for sw in data['topology']['switches']:
            one_switch = Switch(sw['number'])
            
            for port in sw['ports']:
                # считываем очереди
                one_port = Port(tuple(port['link']), port['bandwidth'])
                for qu in port['queues']:
                    one_queue = Queue(qu['priority'], qu['queue_number'], qu['weight'], slices[qu['queue_number']])
                    # заполняем соответствие "слайс" - "очередь"
                    one_switch.slice2port[qu['queue_number']] = tuple(port['link'])
                    one_port.slice_distribution[qu['queue_number']] = one_queue.number
                    # добавляем словарь с очередями "номер очереди" - "очередь"
                    one_port.queues_info[one_queue.number] = one_queue
                    # добавляем информацию о приоритетах очередей: "приоритет" - "очередь"
                    one_port.queue_priority_distr.setdefault(one_queue.priority, [])
                    one_port.queue_priority_distr[one_queue.priority].append(one_queue.number)
                    one_port.virt_time_param[one_queue.priority] = VirtualTime()
                # добавляем порт
                one_switch.port_dict[tuple(port['link'])] = one_port
                # заполняем топологию по формату "номер коммутатора" - "коммутатор"
                # print(one_switch.queues_info)
            topology[one_switch.number] = one_switch

        # считываем каналы и устанавливаем связи между коммутаторами
        # не влияет на программу...
    ...
    for key in slices.keys():
        ...
        for flow in sls.flows_list:
            ...
            while t < T:
                ...
                event_time.add_event(Event(State.ARRIVAL, t, arrival_packet, flow.path[0], (flow.path[0], flow.path[1])))
...
def simulate(event_time, topology, stat):
    ...
    while event != 0:
        ...
        if isinstance(event.port, tuple):
            port = sw.port_dict[event.port]
        else:
            port = event.port
        # except KeyError:
             
        # slice_number = event.packet.slice
        # next_sw = -1
        # try:
        # port = sw.port_dict[sw.slice2port[slice_number]]
        # except KeyError:
            # next_sw = 0
        ...
        if event.state == State.ARRIVAL:
            ...
            if port.id[1] != -1:
                queue_number = port.slice_distribution[event.packet.slice]
                # добавляем пакет в очередь
                port.push_packet_to_queue(queue_number, event.packet, event.time)
                # проверям состояние виртулаьного времени
                port.check_virtual_time_correct(queue_number, event.time)

        # если канал свободен, выполни передачу пакета
        if port.link_state:
            # достаем пакет из очереди, которая имеет больший приоритет на передачу
            packet_prev = event.packet
            packet = port.get_packet_for_transmit()
            ...
            port.check_virtual_time_correct(port.slice_distribution[packet.slice], event.time)
            # вычисляем время окончания отправки пакета
            duration = float(packet.size) / port.bandwidth
            # print("duration :", duration, ", packet:", packet.size, ", bandwidth :", sw.bandwidth)
            # ставим флаг занятости канала передачи
            port.link_state = False
            # добавляем новое событие на текущем коммутаторе
            # print("\n4 --- Create new event on sw", sw.id, "packet time", packet.begin_time)
            event_time.add_event(Event(State.SEND, event.time + duration, packet_prev, sw.number, port.id))
            # создаем событие на следующем коммутаторе
            next_sw = get_next_switch(packet, sw.number)
            try:
                next_port = topology[next_sw].slice2port[event.packet.slice]
            except KeyError:
                next_port = None
            if next_port != None:
                # print("\n4 --- Create new event on sw", next_sw, "packet time", packet.begin_time)
                event_time.add_event(Event(State.ARRIVAL, event.time + duration + transmit, packet, next_sw, next_port))
            else:
            ...
            stat.data_voluem[sw.number][packet.slice] += packet.size
            # и вычисляем текущую пропускную способность
            stat.throughput[sw.number][packet.slice].append(
                stat.data_voluem[sw.number][packet.slice] / event.time + duration + transmit)
        event = event_time.get_time()
...
def main(argv):
    ...
    # output_file = "gene_out" + argv[0][6:len(argv[0])-5]
